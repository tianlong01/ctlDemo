...
public class Zhao2{
        ...
        public static void changeCoins(int[] coins,int money) {
            ...
            Map<Integer,List<HashMap<Integer,Integer>>> coinChangeMap = new HashMap<Integer,List<HashMap<Integer,Integer>>>();

            for (int cents = 1; cents <= money; cents++) {
                // 当用最小币值的硬币找零时，所需硬币数量最多
                int minCoins = cents;
                HashMap<Integer,Integer> minCoinMap = new HashMap<Integer,Integer>();//保存各个面值的具体找零方案
                List<HashMap<Integer,Integer>> minCoinList = new ArrayList<>();
                minCoinMap.put(1, cents);
                minCoinList.add(minCoinMap);
                coinChangeMap.put(cents, minCoinList);
                ...
                for (int kind = 0; kind < valueKinds; kind++) {
                    ...
                    if (coins[kind] <= cents) {  // 若当前面值的硬币小于当前的cents则分解问题并查表
                        ...
                        if (tmpCount <= minCoins) {
                            //子问题的最优解集合
                            List<HashMap<Integer, Integer>> subMaps = coinChangeMap.get(oppCoinVal);
                            HashMap<Integer,Integer> tmpMap = new HashMap<Integer,Integer>();
                            if(subMaps != null && subMaps.size() != 0){
                                for (HashMap<Integer, Integer> subMap : subMaps) {
                                    ...
                                    minCoins = tmpCount;
                                    minCoinMap = tmpMap;
                                    if(!minCoinList.contains(minCoinMap)){
                                        minCoinList.add(minCoinMap);
                                    }
                                }
                            }else{
...
//                            minCoinMap = tmpMap;
        ...
        public static void main(String[] args) {
            // 硬币面值预先已经按降序排列
            int[] coinValue = new int[] {8,7, 5, 4,1 };
            // 需要找零的面值
            int money = 10;
